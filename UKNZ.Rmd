...
prj <- rgcam::loadProject(proj = "data/uk3marchNZ.dat")
...
for (i in seq_along(queries)) {
  ...
  prj[["base"]][[queries[i]]][["scenario"]] <- "Reference"
  prj[["nz25feb"]][[queries[i]]][["scenario"]] <- "Net_Zero"
...
gdp_uk <- getQuery(prj,"GDP by region") %>% mutate(category="GDP",value=value/1000,Units="billion1990USD") %>% filter(region=="UK",year > 2010)                                                         

ei <- left_join(emi_tot,gdp_gj,by=c("year","scenario")) %>% 
  mutate(EmIntes=value.x/value.y)

ggplot()+
geom_line(data = ei %>% filter(year>2015),mapping=aes(x = year,y=EmIntes,color=scenario),size=1.2) +
   scale_color_manual(values = my_custom_palette[c(4,6)])+
  theme(axis.text.x = element_text(size = 12),
         legend.title = element_blank(),
        legend.position="top",
        legend.justification = c(0,1))+
    labs(x= "",y="Emission Intensity (MtCO2/GDP)")
...
geom_area(elec_gen %>% filter(year>2015),mapping=aes(x=year,y=value*277.78,fill=subsector))+
 ...
 #theme_pubclean(base_size = 14)+
  scale_fill_manual(values = my_custom_palette[c(2, 3,4, 6, 8, 10, 12)])+
