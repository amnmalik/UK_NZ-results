...
prj <- rgcam::loadProject(proj = "data/uk11marchNZ.dat")
...
for (i in seq_along(queries)) {
  ...
  prj[["nz11march"]][[queries[i]]][["scenario"]] <- "Net_Zero"
...
gdp_uk <- getQuery(prj,"GDP by region") %>% mutate(category="GDP",value=value/1000,Units="billion1990USD") %>% filter(region=="UK",year > 2000) %>% 
...
ggplot() +  
  geom_line(data = ei %>% filter(year > 2015),   
  ...
  theme(axis.text.x = element_text(size = 12),  
        ...
        legend.justification = c(0, 1)) +  
  labs(x = "",   
       y = expression("Emission Intensity (MtCO"[2]*"/GDP"[italic("2020 USD billion")]*")"))  # Use expression() with subscript  
...
elec_gen <- read_excel("data/elec gen_UK-chetna 28Mar25.xlsx",sheet = "Final_sheet_for_plotting") %>% 
  mutate(Units="TWh") %>% 
...
w <- ggplot() +  
  geom_area(data = elec_gen %>% filter(year > 2015),  
...
z <- ggplot(elec_gen %>% mutate(year=as.character(year)) %>% filter(year %in% c(2020,2050,2070)), mapping=aes(x=year,y=percent,fill=sector))+
  geom_col(size=1.2,show.legend = F)+
      facet_wrap(~scenario)+
   geom_text(mapping=aes(label = ifelse(percent > 5, paste0(round(percent, 0), "%"), "")), 
            position = position_stack(vjust = 0.5), 
            size = 3) +
...
fe3 <- getQuery(prj,"final energy consumption by aggregate sector and fuel") %>% 
  filter(region=="UK",year>2010) %>%    group_by(scenario,sector,year,Units) %>% 
  summarise(value=sum(value)) %>% 
  ungroup() 
