...
prj <- rgcam::loadProject(proj = "data/UKNZ3feb.dat")
...
for (i in seq_along(queries)) {
  ...
  prj[["Baseline"]][[queries[i]]][["scenario"]] <- "Reference"
  prj[["Netzero"]][[queries[i]]][["scenario"]] <- "Net_Zero"
...
ggplot(socioeco %>% filter(region %in% c("UK", "TN", "KA", "MH"), category == "Pop",year >2005,year<2075,scenario=="Reference"), mapping = aes(x = year, y = value / 1e3, color = region)) +
  ...
  geom_label_repel(socioeco %>% filter(region %in% c("UK", "TN", "KA", "MH"), category == "Pop",year==2070,scenario=="Reference"), mapping = aes(x = year, y = value / 1e3, color = region,label = region),
...
ggplot(socioeco %>% filter(region %in% c("UK", "TN", "KA", "MH"), category == "GDPPC", year > 2005, year < 2075,scenario=="Reference"), 
  ...
  geom_line(data = socioeco %>% filter(region %in% c("UK", "TN", "KA", "MH"), category == "GDPPC", year > 2005, year <= 2020,scenario=="Reference"), 
            aes(x = year, y = value * 1000 * 1.98, color = region), 
            linetype = "solid", size = 1.2) +
  geom_point(shape="circle",size=2)+
  geom_line(data = socioeco %>% filter(region %in% c("UK", "TN", "KA", "MH"), category == "GDPPC", year >= 2020, year < 2075,scenario=="Reference"), 
            aes(x = year, y = value * 1000*1.98, color = region), 
            linetype = "dashed", size = 1.2) +
   geom_label_repel(socioeco %>% filter(region %in% c("UK", "TN", "KA", "MH"), category == "GDPPC",year==2070,scenario=="Reference"), mapping = aes(x = year, y = value*1000*1.98, color = region,label = region),
    nudge_x = 6, box.padding = 0.3, direction = "y",
    na.rm = TRUE, show.legend = F,force_pull = 0.5)+
  theme_pander(boxes = TRUE, base_size = 14) +
  scale_color_aaas() +
...
writexl::write_xlsx(x = emi,"output_data/AggregateEmissions.xlsx")
...
geom_area(data = emi %>% filter(year>2015),mapping=aes(x = year,y=value*3.67,fill=sector))+
  ...
  labs(y = expression(Emissions~(MTCO[2])),x="",caption = "Indsutry emissions includes emission from fuel processing")+
...
gdp <- getQuery(prj,"GDP by region") %>% mutate(category="GDP") %>% 
  filter(region=="UK") %>% select(scenario,year,value) %>% mutate(title="GDP")
pop <- getQuery(prj,"Population by region") %>% mutate(category="Pop")%>% 
  filter(region=="UK") %>% select(scenario,year,value) %>% mutate(title="Population")
gdpPerCap <- getQuery(prj,"PPP GDP by region") %>% mutate(category="GDPPC")%>% 
  filter(region=="UK") %>% select(scenario,year,value) %>% mutate(title="GDP Per capita")
...
gdp_gj <- getQuery(prj,"GDP by region") %>% mutate(category="GDP") %>% filter(region=="UK",year > 2000)                                                         
...
fe3 <- getQuery(prj,"final energy consumption by aggregate sector and fuel") %>% 
  filter(region=="UK") %>% 
  # agriculture energy use is considered in industry
...
writexl::write_xlsx(energy_intensity,"output_data/energy_intensity.xlsx")
energy_intensity <- readxl::read_excel("output_data/energy_intensity.xlsx")
...
```{r}
gdp_gj <- getQuery(prj,"GDP by region") %>% mutate(category="GDP") %>% filter(region=="UK",year > 2010)      %>% 
...
elec <- getQuery(prj,"final energy consumption by aggregate sector and fuel") %>% 
  filter(region=="UK",input=="electricity") %>% 
  group_by(scenario,sector,year) %>% 
    summarise(value=sum(value)) %>% 
  group_by(year,scenario) %>% 
  mutate(percent=value/sum(value),percent=percent*100)
...
writexl::write_xlsx(elec,"output_data/elec.xlsx")
...
######################################
ggplot(elec %>% filter(year %in% c(2020, 2050, 2070)),mapping=aes(x=as.factor(year),y=percent,fill=sector))+
...
###########################
  ggplot()+
  geom_col(elec_gen %>% filter(year %in% c(2020,2030,2040,2050)), mapping=aes(x=year,y=percent,fill=subsector,group=interaction(scenario,subsector)),size=1.2)+
    facet_wrap(~scenario) +
...
writexl::write_xlsx(fe %>% filter(year %in% c(2020, 2050, 2070)),"output_plot/fe_shares.xlsx")
...
writexl::write_xlsx(trans,"output_data/transport_demand.xlsx")
...
trans_fuel <- getQuery(prj,"transport final energy by fuel") %>% 
  filter(region=="UK",year>2015) %>% mutate(scenario=as.factor(scenario)) %>% 
  group_by(scenario,year) %>% 
...
blah <- build4 %>% select(-percent) %>% 
  filter(year %in% c(2020,2050), scenario=="Reference",!sector%in% c("Heating","Others")) %>% 
  pivot_wider(names_from = year,values_from = value) %>% 
...
floorspace <- getQuery(prj,"Building floorspace") %>% 
  filter(region=="UK",year>2015,year<2075) %>%  group_by(scenario, building) %>%
...
industry <- getQuery(prj,"final energy consumption by sector and fuel") %>% 
  filter(region=="UK",sector!="Agriculture_enuse") %>% 
...
#writexl::write_xlsx(industry,"output_data/industry_FE.xlsx")
